...
int starter_cmp_conn (struct starter_conn *c1, struct starter_conn *c2)
{
	...
	STRCMP(name);
	VARCMP(policy);
	VARCMP(options[KNCF_IKELIFETIME]);
	VARCMP(options[KNCF_SALIFETIME]);
	VARCMP(options[KNCF_REKEYMARGIN]);
	VARCMP(options[KNCF_REKEYFUZZ]);
	VARCMP(options[KNCF_KEYINGTRIES]);
	if (starter_cmp_end(&c1->left,&c2->left)) return -1;
	if (starter_cmp_end(&c1->right,&c2->right)) return -1;
	VARCMP(options[KNCF_AUTO]);
	STRCMP(esp);
	STRCMP(ike);
	return 0;
}
