...
select_x509cert_id(x509cert_t *cert, struct id *end_id)
{
	    ...
	    end_id->kind != ID_FROMCERT)
	{
	     ...
	     libreswan_log("  no subjectAltName matches ID '%s', replaced by subject DN", buf);
	}
	end_id->kind = ID_DER_ASN1_DN;
		...
		const x509cert_t *issuer_cert)
{
    ...
    {
	libreswan_log("  digest algorithm not supported");
	return FALSE;
    }
...
gntoid(struct id *id, const generalName_t *gn)
{
    ...
    case GN_IP_ADDRESS:		/* ID type: ID_IPV4_ADDR */
	{
	    ...
	    if (!ugh)
		{
		 libreswan_log("Warning: gntoid() failed to initaddr(): %s", ugh);
		}
...
parse_authorityInfoAccess(chunk_t blob, int level0, chunk_t *accessLocation)
{
	...
	case AUTH_INFO_ACCESS_LOCATION:
		...
		case OID_OCSP:
		    ...
		    libreswan_log("warning: ignoring OCSP InfoAccessLocation with unknown protocol");
		    break;
		default:
