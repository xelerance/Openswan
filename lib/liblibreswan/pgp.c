...
parse_pgp_pubkey_packet(chunk_t *packet, pgpcert_t *cert)
{
	...
	{
	    libreswan_log("  computation of V4 key ID not implemented yet");
	}
	break;
...
parse_pgp_secretkey_packet(chunk_t *packet, rsa_privkey_t *key)
{
    ...
    if (s2k == 255)
    {
	libreswan_log("  string-to-key specifiers not supported");
	return FALSE;
    }

    if (s2k >= PGP_SYM_ALG_ROOF)
    {
	libreswan_log("  undefined symmetric key algorithm");
	return FALSE;
    }
    ...
    libreswan_log("  %s encryption not supported",  pgp_sym_alg_name[s2k]);
    return FALSE;
}
...
parse_pgp(chunk_t blob, pgpcert_t *cert, rsa_privkey_t *key)
{
	...
	if (!(packet_tag & 0x80))
	{
	    libreswan_log("  incorrect Packet Tag");
	    return FALSE;
	}
	...
	if (packet_tag & 0x40)
	{
	    libreswan_log("  new PGP packet format not supported");
	    return FALSE;
	}
	else
	{
