...
/* in testing, we do not want randomness */
void get_random_bytes(void *buf, int nbytes)
{
  prng_bytes(&ipsec_prng, buf, nbytes);
}

int
pfkey_list_insert_supported(struct supported *supported,
...
int
pfkey_list_remove_supported(struct supported *supported,
			    struct supported_list **supported_list)
{
  return -1;
}
