...
find_raw_ifaces4(void)
{
    ...
    /* Add an entry to rifaces for each interesting interface. 
       ...
       the union. See if.h */
    for (bp = buf, j = 0; bp < (unsigned char *)buf + (size_t)ifconf.ifc_len;
     bp = (struct ifreq *) ((unsigned char *)bp + _SIZEOF_ADDR_IFREQ(*bp)), j++)
    {
	struct raw_iface ri;
	const struct sockaddr_in *rs = (struct sockaddr_in *) &bp->ifr_addr;
...
bool
do_command_darwin(struct connection *c, struct spd_route *sr
